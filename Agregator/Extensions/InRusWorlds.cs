...
namespace Agregator.Extensions
{

    public static class Extensions
    {
        public static async Task<JToken> GetBody(this Stream body)
        {
            var bodyData = await new StreamReader(body).ReadToEndAsync();
            return (JToken)JsonConvert.DeserializeObject(bodyData);
        }
    }
        ...
        static Dictionary<TextCase, string[]> monthNames = new Dictionary<TextCase, string[]>
    {
        ...
        /// <returns></returns>
        public static string DateToTextLong(this DateTime _date, string _year=null)
        {
            return String.Format("«{0}» {1} {2}",
                                    _date.Day.ToString("D2"),
                                    MonthName(_date.Month, TextCase.Genitive),
                                    _date.Year.ToString()) + (string.IsNullOrEmpty(_year) ? string.Empty:$" {_year}");
        }
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        public static string DateToTextLong(this DateTime _date)
        {
            return String.Format("«{0}» {1} {2}",
                                    _date.Day.ToString("D2"),
                                    MonthName(_date.Month, TextCase.Genitive),
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        public static string DateToTextQuarter(this DateTime _date)
        {
        ...
        /// <param name="_date"></param>
        /// <returns></returns>
        public static string DateToTextSimple(this DateTime _date)
        {
            return String.Format("{0:dd.MM.yyyy}", _date);
        }
        public static int DateQuarter(this DateTime _date)
        {
        ...
        /// <returns></returns>
        public static string CurrencyToTxt(this double _amount, bool _firstCapital)
        {
            //Десять тысяч рублей 67 копеек
        ...
        /// <param name="_firstCapital"></param>
        /// <returns></returns>
        public static string CurrencyToTxtFull(this double _amount, bool _firstCapital)
        {
            //10 000 (Десять тысяч) рублей 67 копеек
            long rublesAmount = (long)Math.Floor(_amount);
            long copecksAmount = ((long)Math.Round(_amount * 100)) % 100;
