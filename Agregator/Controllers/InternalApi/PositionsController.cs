...
using Position = Agregator.Data.Position;
...
namespace WebApp.Controllers
{
    ...
    public class PositionsController : Controller
    {
        ...
        public async Task<IActionResult> Put(int key, string values) {
            var model = await _context.Positions.FirstOrDefaultAsync(item => item.RowId == key);
            if(model == null)
        ...
        private void PopulateModel(Position model, IDictionary values) {
            string ROW_ID = nameof(Position.RowId);
            string NAME = nameof(Position.Name);
